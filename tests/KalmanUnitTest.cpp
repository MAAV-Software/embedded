...
BOOST_AUTO_TEST_CASE(ctorTest)
{
    ...
    for (uint16_t r = 0; r < x.numRows; ++r)
    {
        for (uint16_t c = 0; c < x.numCols; ++c)
            BOOST_CHECK_CLOSE(MaavMath::mat_noref_at(x, r, c), 0.0, 0.0001);
    }
    
    for (uint16_t r = 0; r < P.numRows; ++r)
    {
        for (uint16_t c = 0; c < P.numCols; ++c)
            BOOST_CHECK_CLOSE(MaavMath::mat_noref_at(P, r, c), 0.0, 0.0001);
    }
}
...
BOOST_AUTO_TEST_CASE(ptest)
{
    ...
    BOOST_CHECK_CLOSE(MaavMath::mat_noref_at(x, 0, 0), 0.00000, 100);
    BOOST_CHECK_CLOSE(MaavMath::mat_noref_at(x, 1, 0), 0.00010, 100);
    BOOST_CHECK_CLOSE(MaavMath::mat_noref_at(x, 2, 0), 0.00000, 100);
    BOOST_CHECK_CLOSE(MaavMath::mat_noref_at(x, 3, 0), 0.01000, 100);
    BOOST_CHECK_CLOSE(MaavMath::mat_noref_at(x, 4, 0), 0.00000, 100);
    BOOST_CHECK_CLOSE(MaavMath::mat_noref_at(x, 5, 0), 0.00001, 100);
