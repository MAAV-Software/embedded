...
int main(void)
{
	...
	for (;;)
	{
		if ((count - led_time) > 10)
		{
			...
			char buffer[100];
			uint32_t len = snprintf(buffer, 100, "Loop = %u\n", count);
		...
		if (lidar_can_transmit == true)
		{
			...
			char buffer[100];
			uint32_t len = snprintf(buffer, 100, "Initiating Transmit\n");
			UARTwrite(buffer, len);
		}
		...
		if (lidar_transmit_done() == true)
		{
			...
			char buffer[100];
			uint32_t len = snprintf(buffer, 100, "Distance = %u\n", distance);
			UARTwrite(buffer, len);
		}
