...
void check_lighted_switches(SwitchData_t *sw[])
{
	int i;
	for (i = 0; i < 3; ++i)
	{
...
uint8_t check_ctrl_mode()
{
	uint8_t mode = automomousMode(servoIn_getPulse(RC_CHAN5)) ? 3 :
...
void update_gains(quad_ctrl_t *qc, float *xy_rateGains[], float *z_valueGains[],
				  uint8_t mode)
{
