...
//    printf("deliver packet, channel %-30s, size %10d\n", channel, buflen);
	...
	lcmlite_subscription_t *sub = lcm->first_subscription;
    for (sub = lcm->first_subscription; sub != NULL; sub = sub->next) {
...
int lcmlite_receive_packet(lcmlite_t *lcm, const void *_buf, int buf_len, uint64_t from_addr)
{
    ...
    if (magic == MAGIC_LCM2) {
        ...
        int idx = 0;
        for (idx = 0; idx < LCM3_NUM_BUFFERS; idx++) {
