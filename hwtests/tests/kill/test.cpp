...
void TestFunction::run()
{
	...
	for(;;)
	{
		...
		if (dutyYawd > 0.5) {
			...
			// If duty cycle value for pitch is greater than 0.6, flash the blue LED
			if (dutyXd > 0.6) {
			...
			// If duty cycle value for roll is greater than 0.5, flash the red LED
			if (dutyYd > 0.5) {
			...
			// If duty cycle value for thrust is greater than 0.5, flash the green LED
			if (dutyFz > 0.5) {
