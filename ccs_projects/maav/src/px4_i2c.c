...
//	}
	...
	else if (g_PX4_State == GET_INTERMEDIATE_BYTES)
	{
		...
		if(idx_px4 == 21)
		{					// this was the second to last byte
			g_PX4_State = GET_LAST_BYTE;
			I2CMasterControl(I2C_Base, I2C_MASTER_CMD_BURST_RECEIVE_FINISH);
		}
		else I2CMasterControl(I2C_Base, I2C_MASTER_CMD_BURST_RECEIVE_CONT);
	}
	...
	else if (g_PX4_State == DATA_READY) g_PX4_State = DATA_READY; // There is no reason I should get here
...
void init_px4_i2c(uint32_t I2C_peripheral, uint32_t I2C_SCL_peripheral,
				  ...
				  uint32_t _I2C_base, uint32_t SCL_port, uint32_t SDA_port,
				  uint32_t SCL_pin, uint32_t SDA_pin, uint32_t SCL_pinConfig,
				  uint32_t SDA_pinConfig)
{
